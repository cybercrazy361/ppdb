...
// --- Ambil notif jika ada dari URL GET (reload) ---
$notif_admin = '';
if (!empty($_GET['notif'])) {
...
// ===================================
// PROSES GENERATE PDF DENGAN MPDF
// ===================================
...
if (!@file_get_contents($pdf_url)) {
    die("PDF TIDAK BISA DIAKSES PUBLIK: $pdf_url. Cek permission/folder/domain!");
        ...
        if (!empty($res['error'])) {
            ...
            $resultObj = json_decode($res['result'], true);
            if (!empty($resultObj['status']) && $resultObj['status']) {
                $notif_admin = "Bukti PDF berhasil dikirim ke WhatsApp orang tua ($no_wa_ortu)";
            } else {
                $notif_admin = "Gagal kirim: " . htmlspecialchars($res['result']);
            }
        }
    }
