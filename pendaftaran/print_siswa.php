...
// ===== STATUS PEMBAYARAN (cek progres) =====
$uang_pangkal_id = 1;
$spp_id = 2;

// Query progres pembayaran (seperti di halaman list)
$query_status = "
    SELECT
        CASE
            WHEN 
...
// ===================
// Ambil histori pembayaran terakhir (optional)
// ===================
$pembayaran_terakhir = [];
$stmtBayar = $conn->prepare("
    SELECT pd.jenis_pembayaran_id, jp.nama AS jenis, pd.nominal, pd.status_pembayaran, pd.bulan, p.tanggal_pembayaran
...
function getStatusBadge($status) {
    switch (strtolower($status)) {
        case 'lunas': return '<span style="color:#1cc88a;font-weight:bold"><i class="fas fa-check-circle"></i> Lunas</span>';
        case 'angsuran': return '<span style="color:#f4b400;font-weight:bold"><i class="fas fa-exclamation-circle"></i> Angsuran</span>';
        default: return '<span style="color:#e74a3b;font-weight:bold"><i class="fas fa-times-circle"></i> Belum Bayar</span>';
    }
}
?>
<!DOCTYPE html>
...
<style>
  ...
  .header {
    position: relative; min-height: 80px; border-bottom: 2px solid #d1d5db;
  ...
  @media print {
    .logo { width: 120px; height: 120px; top: -25px; }
  ...
  .status-row {
    font-size: 17px; font-weight: bold; color: #222; text-align: center;
    background: #f6f7fc; border-radius: 7px; padding: 12px 0 8px 0; margin-bottom: 20px;
  ...
  .footer-ttd-kanan {
    ...
    margin-top: 65px; min-height: 90px;
  }
  .ttd-block-kanan {
  ...
  @media print {
    ...
    .container {
      width: 100% !important; max-width: 185mm !important; min-height: 250mm;
      margin: 0 auto !important; border-radius: 0 !important;
      box-shadow: none !important; border: none !important; page-break-after: avoid;
    }
    .no-print { display: none !important; }
    .data-table caption, .data-table th, .note {
      -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important;
    }
...
<body>
  ...
  <div class="container">
    ...
    <!-- TAMPILKAN STATUS PEMBAYARAN SEKARANG -->
    ...
    <!-- OPSIONAL: TABEL PEMBAYARAN TERAKHIR -->
    <?php if ($status_pembayaran != 'Belum Bayar'): ?>
      <div style="margin:18px 0 4px 0;font-size:15.2px;font-weight:500;">Riwayat Pembayaran Terakhir:</div>
