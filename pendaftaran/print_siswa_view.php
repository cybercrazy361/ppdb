...
<?php if (!isset($_GET['send_wa'])): ?>
  ...
  <button id="btnWA" onclick="window.location.href='<?= $_SERVER['PHP_SELF'] .
      '?id=' .
      $id .
      '&send_wa=1' ?>'" style="display:inline-block;margin:10px 0 16px 12px;padding:7px 18px;font-size:14px;background:#25d366;color:#fff;border:none;border-radius:6px;cursor:pointer;">
...
<?php endif; ?>
  ...
  <div class="container">
    ...
    <table class="data-table">
      ...
      <tr><th>Nama Calon Peserta Didik</th><td><?= safe(
          $row['nama']
      ) ?></td></tr>
    ...
    <div class="note <?= $note_class ?>">
      <?php if ($status_pembayaran === 'Belum Bayar'): ?>
        ...
        2. Formulir ini **bukan bukti diterima**, siswa diterima jika sudah lunas & dapat nomor formulir.
      <?php elseif ($status_pembayaran === 'Angsuran'): ?>
        <b>Catatan:</b><br>
        Siswa sudah membayar sebagian.<br>
        Simpan bukti ini, lakukan pelunasan agar status menjadi LUNAS.
      <?php elseif ($status_pembayaran === 'Lunas'): ?>
        <b>Catatan:</b><br>
        Siswa sudah **lunas** seluruh pembayaran administrasi.<br>
        Simpan bukti ini sebagai konfirmasi pendaftaran.
      <?php else: ?>
        <b>Catatan:</b><br>
