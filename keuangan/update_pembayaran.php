...
$pembayaran_id = isset($input['pembayaran_id'])
    ? intval($input['pembayaran_id'])
    : 0;
$metode_pembayaran = isset($input['metode_pembayaran'])
    ? sanitize($input['metode_pembayaran'])
    : '';
$tahun_pelajaran = isset($input['tahun_pelajaran'])
    ? sanitize($input['tahun_pelajaran'])
    : '';
$keterangan = isset($input['keterangan']) ? sanitize($input['keterangan']) : '';

$jenis_list =
    isset($input['jenis_pembayaran']) && is_array($input['jenis_pembayaran'])
        ? $input['jenis_pembayaran']
        : [];
$jumlah_list =
    isset($input['jumlah_pembayaran']) && is_array($input['jumlah_pembayaran'])
        ? $input['jumlah_pembayaran']
        : [];
$bulan_list =
    isset($input['bulan_pembayaran']) && is_array($input['bulan_pembayaran'])
        ? $input['bulan_pembayaran']
        : [];
$cb_list =
    isset($input['cashback']) && is_array($input['cashback'])
        ? $input['cashback']
        : [];
...
try {
    ...
    if ($res->num_rows === 0) {
        throw new Exception(
            'Pembayaran tidak ditemukan atau tidak sesuai unit.'
        );
    }
    $row = $res->fetch_assoc();
    ...
    for ($i = 0; $i < count($jenis_list); $i++) {
        ...
        if ($jenis_id === '') {
            $errors[] =
                'Item ' . ($i + 1) . ': Jenis pembayaran harus dipilih.';
            continue;
        }
        if ($effective <= 0) {
            $errors[] =
        ...
        if ($res_jenis->num_rows === 0) {
            $errors[] =
                'Item ' .
                ($i + 1) .
                ': Jenis pembayaran tidak ditemukan untuk unit Anda.';
            $stmt->close();
            continue;
        }
        ...
        if ($nama_jenis === 'spp') {
            ...
            for ($k = 0; $k < count($jenis_list); $k++) {
                if ($jenis_list[$k] == $jenis_id) {
                    $stmt_spp = $conn->prepare(
                        'SELECT nama FROM jenis_pembayaran WHERE id=?'
                    );
            ...
            if (
                $bulan !== $first_unpaid &&
                !in_array($first_unpaid, $bulan_spp_in_update)
            ) {
                $errors[] =
                    'Item ' .
                    ($i + 1) .
            ...
            if ($nama_jenis !== 'uang pangkal' && $cb > 0) {
                $errors[] =
                    'Item ' . ($i + 1) . ': Cashback hanya untuk Uang Pangkal.';
            }
            if ($nama_jenis === 'uang pangkal' && $cb < 0) {
        ...
        if ($nama_jenis === 'spp') {
            if ($prev >= $nominal_max) {
                $errors[] =
                    'Item ' .
                    ($i + 1) .
                    ": SPP bulan $bulan sudah lunas, tidak bisa bayar lagi.";
                continue;
            }
            if ($effective > $sisa) {
                $errors[] =
    ...
    // Hapus detail lama
    $stmt = $conn->prepare(
        'DELETE FROM pembayaran_detail WHERE pembayaran_id = ?'
    );
    ...
    for ($i = 0; $i < count($jenis_list); $i++) {
        ...
        $cb =
            isset($cb_list[$i]) && $cb_list[$i] !== ''
                ...
                : 0;
        $bulan =
            isset($bulan_list[$i]) && $bulan_list[$i] !== ''
                ? $bulan_list[$i]
                : null;
        $effective = $jumlah + $cb;
